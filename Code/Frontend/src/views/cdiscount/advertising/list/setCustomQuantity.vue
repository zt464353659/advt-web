  ...
  >
    <template>
      <el-form ref="formData" :model="formData" :rules="rules" label-width="100px" size="small" @submit.native.prevent>
        <el-form-item label="指定库存" prop="custom_quantity">
          <el-input
            type="text"
            clearable
            style="width: 90%;"
        ...
        <!--        <el-form-item label="在线天数" prop="effective_days">-->
        <!--          <el-input-->
        <!--            type="text"-->
        <!--            clearable-->
        <!--            style="width: 90%;"-->
...
<script>
import { setCustomStock } from '@/api/cdiscount'
import { resultAlert } from '@/utils/index'

export default {
  data() {
    const checkCustomQuantity = (rule, value, callback) => {
      if (!value) {
        return callback(new Error('指定库存不能为空'))
      }
      setTimeout(() => {
        const quantityReg = /(^\d*$)/
        if (!quantityReg.test(value)) {
          callback(new Error('指定库存必须为0或正整数'))
        }
        callback()
      }, 100)
    }
    ...
    return {
      ...
      rules: {
        custom_quantity: [
          { required: true, validator: checkCustomQuantity, trigger: 'blur' }
        ]
        // effective_days: [
    ...
    setPrice(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this._.forEach(obj.product_id, val => {
            obj.custom_stock[val] = this.formData.custom_quantity
          })
          setCustomStock(obj).then((res) => {
            //调用弹窗
            if (res.data) {
              resultAlert(res)
            } else {
