...
<script>
  import { editPromo } from '@/api/cdiscount'
  ...
  export default {
    ...
    methods: {
      ...
      // 数据回填处理
      dataDispose(val) {
        this.formData.sale_name = val.sale_name
        ...
        this.formData.activity_date = [this.getLocalDate(val.start_time), this.getLocalDate(val.end_time)]
        this.selectDate(this.formData.activity_date)
        this.formData.start_time = this.getLocalTime(val.start_time)
        this.formData.end_time = this.getLocalTime(val.end_time)
      }
    },
    filters: {},
    watch: {
