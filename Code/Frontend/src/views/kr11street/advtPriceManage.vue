  ...
  <div style="width: 100%">
    ...
    <div class="header-box">
      <el-form ref="listQuery" :model="listQuery" size="mini" :inline="true" class="advt-form-inline">
        <el-form-item label="Site Code" prop="accountId">
          <el-select v-model="listQuery.accountId" placeholder="请选择" clearable multiple collapse-tags style="width: 180px;margin-left: 20px;">
      ...
      <el-row class="right-row">
        <el-button type="primary" size="mini" @click="openTask" v-permission="permissions.setFellow">添加任务</el-button>
      </el-row>
    </div>
    <!--  列表  -->
    <div class="content-box">
      ...
      >
        ...
        <el-table-column label="Site code" prop="site_code" width="90"></el-table-column>
        ...
        <el-table-column label="状态" prop="is_enable" width="60" align="center">
          <template slot-scope="scope">
            <el-tag :type="Number(scope.row.is_enable)===1?'success':'danger'" size="mini">{{ Number(scope.row.is_enable) === 1 ? '启用' : '禁用' }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="添加人" prop="user_name" width="90"></el-table-column>
        <el-table-column label="添加时间" prop="create_time" width="150"></el-table-column>
        <el-table-column label="操作" width="150" align="center">
          <template slot-scope="scope">
            <el-button type="text" v-permission="permissions.onOffFellow" size="mini" @click="Switchoperation(scope.row)">{{ Number(scope.row.is_enable) === 1 ? '禁用' : '启用' }}</el-button>
            <el-button type="text" v-permission="permissions.updateFellow" size="mini" @click="ModifyTask(scope.row)">编辑</el-button>
            <el-button type="text" size="mini" @click="showDetails(scope.row)">详情</el-button>
            <el-button type="text" v-permission="permissions.followLog" size="mini" @click="openLog(scope.row)">日志</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!--分页-->
    ...
    <task v-bind.sync="logTask" @renderList="getfollowList"></task>
    <taskeditor v-bind.sync="logEditor" @renderList="getfollowList"></taskeditor>
...
import { getSelectAll, followList, onOffFellow } from '@/api/kr11street'
...
export default {
  ...
  methods: {
    ...
    getfollowList() {
      ...
      followList(queryParams).then(res => {
