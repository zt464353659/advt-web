  ...
  <div>
    ...
    <div class="header-box">
      ...
      <el-row class="right-row">
        <el-button type="primary" size="mini" icon="el-icon-circle-plus-outline" v-permission="permissions.plan_AddThePlan" @click="addPlan">添加计划</el-button>
      </el-row>
    </div>
    <!-- 列表 -->
...
<script>
  import { fetchPlanList, getSelectAll } from '@/api/daraz'
  ...
  export default {
    ...
    data() {
      return {
        permissions:{
          plan_AddThePlan:"daraz.platform.product-upload-plan.add",//添加计划按钮
          plan_PlanDetails:"daraz.platform.product-upload-plan.info"//添加计划按钮
        },//权限
        ...
        options: {},
        /* 上传计划搜索条件列表 */
        selectOptions: ['DarazAdvtAccount']
      }
    },
    created() {
      this.getList()
    ...
    methods: {
      ...
      addPlan() {
        this.addPlanDialogOption = {
          open: true,
          options: this.options.DarazAdvtAccount
        }
      },
      getPlanDetails(data) {
