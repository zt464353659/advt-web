<template>
  ...
  >
    <template>
      <el-form :inline="true" :data="listQuery" :rules="rules" ref="listQuery" :model="listQuery" size="mini" @submit.native.prevent>
...
<script>
import { getPlanDetail } from '@/api/blibli'
...
export default {
  ...
  methods: {
    getDetails() {
      ...
      getPlanDetail(this.listQuery).then(response => {
