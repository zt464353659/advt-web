...
<script>
  import { getSelectAll, productUpdateList, cancleProductNoUpdate } from '@/api/linio'
  import addForm from './noUpdate/addNoUpdate'
  import { filterQueryParams } from '@/utils/help'
  import { resultAlert } from '@/views/linio/advertising/common'
  ...
  export default {
    ...
    data() {
      return {
        permissions: {
          cancle: 'linio.advt.update-rule.cancel',
          add: 'linio.advt.update-rule.set'
        },
    ...
    methods: {
      transformType(row) {
        let str = ''
        row.comment.forEach(v => {
          this.updateOptions.forEach(k => {
            if (Number(v) === k.key) {
              str += `${k.label}  `
            }
          })
      ...
      searchInit() {
        ...
        getSelectAll({ keys: optionsParams }).then(response => {
          this.options = response.data.LinioAdvtAccount
        })
      },
      getList() {
        this.listData = []
