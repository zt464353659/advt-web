...
export function resultAlert(res, nextFun) {
  ...
  Vue._.forEach(list, item => {
    if (item.status) {
      msg.text += `<i class="el-icon-success" style="color:#67c23a"></i>ID:${item.istore_product_id}`
      msg.status = true
    } else {
      msg.text += `<i class="el-icon-error" style="color:#f56c6c"></i>ID:${item.istore_product_id}`
    }
