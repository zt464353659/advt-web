    ...
    <div class="header-box">
      <el-form ref="listQuery" :inline="true" :model="listQuery" class="demo-form-inline" size="mini">
        ...
        <el-form-item label="Product ID" prop="product_id">
          <el-input size="mini" v-model="listQuery.product_id" clearable placeholder="多个请用空格分隔"></el-input>
        </el-form-item>
...
<script>
  import { getSelectAll, productUpdateList, cancleProductNoUpdate } from '@/api/tiki'
  ...
  export default {
    ...
    data() {
      return {
        permissions: {
          cancle: 'tiki.advt.advt.update-rule.cancel',
          add: 'tiki.advt.advt.update-rule.set'
        },
    ...
    methods: {
      searchInit() {
        const optionsParams = ['TikiAdvtAccount', 'TikiAdvtTypes']
        getSelectAll({ keys: optionsParams }).then(response => {
          this.options = response.data.TikiAdvtAccount
          this.advtTypeOptions = response.data.TikiAdvtTypes
        })
      },
      getList() {
        this.listData = []
