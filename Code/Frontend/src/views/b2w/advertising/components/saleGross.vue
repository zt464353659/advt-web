<template>
  <el-dialog :title="'设置促销毛利率'" v-dragMove :visible="open" :before-close="onClose" width="600px" v-loading="loading" :close-on-press-escape="false" :close-on-click-modal="false">
    <el-form ref="form" label-width="120px" :model="form" :rules="rules" size="small">
      <el-form-item label="促销毛利率" prop="gross_margin">
...
<script>
  import { changeSalePrice } from '@/api/b2w'
  ...
  export default {
    data() {
      const checkPrice = (rule, value, callback) => {
        ...
        if (!reg.test(value)) {
          callback(new Error('促销毛利率必须是0-100之间的数字'))
        } else {
          callback()
        }
      }
