      ...
      <el-table :key="newDate" :data="detailsData" border v-loading="listLoading" max-height="500" class="table-gray" style="width: 100%">
        ...
        <el-table-column prop="status" label="状态" width="120">
          <template slot-scope="scope">
            ...
            <el-tag type="danger" size="small" v-if="Number(scope.row.status) === 20">执行出错</el-tag>
            <el-tag type="success" size="small" v-if="Number(scope.row.status) === 30">执行成功</el-tag>
            <el-tag size="small" v-if="Number(scope.row.status) === 40">正在执行</el-tag>
          </template>
        </el-table-column>
...
<script>
import { fetchDetails } from '@/api/real'
