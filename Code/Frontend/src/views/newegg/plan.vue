  ...
  <div>
    ...
    <div class="header-box">
      <el-form ref="listQuery" :inline="true" :model="listQuery" class="demo-form-inline" size="mini">
        <el-form-item label="Site Code" prop="accountId">
          <el-select v-model="listQuery.accountId" clearable placeholder="请选择账号">
    ...
    <div class="content-box">
      ...
      >
        ...
        <el-table-column prop="site_code" label="Site Code" width="170" align="center"></el-table-column>
        <el-table-column prop="data" label="产品 ID">
          <template slot="header" slot-scope="scope">
            <span v-if="scope" style="display:inline-block;text-align:center;width:100%;">产品 ID</span>
          </template>
        ...
        <el-table-column prop="type" width="120" label="类型" align="center">
          <template slot-scope="scope">
            ...
            <el-tag type="primary" size="small" v-else-if="Number(scope.row.type) === 2">激活广告</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="status" width="120" label="状态" align="center">
          <template slot-scope="scope">
            <el-tag type="info" size="small" v-if="scope.row.status === '未执行'">未执行</el-tag>
            <el-tag type="danger" size="small" v-else-if="scope.row.status === '执行出错'">执行出错</el-tag>
            <el-tag type="success" size="small" v-else-if="scope.row.status === '执行成功'">执行成功</el-tag>
            <el-tag type="primary" size="small" v-else-if="scope.row.status === '正在执行'">正在执行</el-tag>
          </template>
        </el-table-column>
...
<script>
import { fetchPlanList, getSelectAll } from '@/api/newegg'
...
export default {
  ...
  data() {
    return {
      permissions: {
        plan_AddThePlan: 'newegg.platform.product-upload-plan.add',//添加计划按钮
        plan_PlanDetails: 'newegg.platform.product-upload-plan.info'//详情
      },//权限
