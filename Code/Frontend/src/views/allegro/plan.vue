    ...
    <div class="header-box">
      <el-form ref="listQuery" :inline="true" :model="listQuery" class="demo-form-inline" size="mini">
        ...
        <el-form-item label="操作人" prop="operator">
          <el-input v-model="listQuery.operator" clearable size="mini" placeholder="请输入操作人"></el-input>
        </el-form-item>
        <el-form-item>
      ...
      >
        ...
        >
          <template slot-scope="scope">
            ...
            <el-tag type="primary" size="small" v-if="Number(scope.row.status) === 15">正在执行</el-tag>
        ...
        <el-table-column prop="create_time" label="操作时间" width="170" sort-by="create_time" sortable>
          <template slot-scope="scope">{{ scope.row.create_time }}</template>
        </el-table-column>
        <el-table-column label="操作" width="80" align="center">
          <template slot-scope="scope">
            <el-button type="text" size="mini" v-if="Number(scope.row.status) !== 10" @click="getPlanDetails(scope.row)" v-permission="permissions.plan_PlanDetails">详情</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!--分页-->
...
<script>
import { apiGetPlanList } from '@/api/allegro'
...
export default {
  ...
  data() {
    return {
      permissions: {
        plan_AddThePlan: 'allegro.advt.product-upload-plan.add',//添加计划按钮
        plan_PlanDetails: 'allegro.advt.product-upload-plan.info'//详情
      },//权限
  ...
  methods: {
    getList() {
      ...
      apiGetPlanList(queryParams).then(response => {
